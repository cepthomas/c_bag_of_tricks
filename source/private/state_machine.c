...
int sm_Destroy(sm_t* sm)
{
    ...
    while(list_IterNext(sm->state_descs, (void**)&st) == CBOT_ERR_NO_ERR)
    {
        //VAL_PTR(st, BAD_PTR);
...
int sm_ProcessEvent(sm_t* sm, unsigned int event_id)
{
    ...
    *ld = event_id;
    ...
    if(!sm->processing_events)
    {
        ...
        int* qevt;
        while (list_Pop(sm->event_queue, (void**)&qevt) == CBOT_ERR_NO_ERR)
        {
            ...
            if(sm->default_state != NULL)
            {
                ...
                while(list_IterNext(sm->default_state->trans_descs, (void**)&trans) == CBOT_ERR_NO_ERR)
                {
            ...
            if(trans_desc == NULL)
            {
                ...
                while(list_IterNext(sm->current_state->trans_descs, (void**)&trans) == CBOT_ERR_NO_ERR)
                {
                    if(trans->event_id == qevtid) // found it
                    {
                        trans_desc = trans;
...
int sm_ToDot(sm_t* sm, FILE* fp)
{
    ...
    while(list_IterNext(sm->state_descs, (void**)&st) == CBOT_ERR_NO_ERR)
    {
        ...
        while(list_IterNext(st->trans_descs, (void**)&trans) == CBOT_ERR_NO_ERR)
        {
